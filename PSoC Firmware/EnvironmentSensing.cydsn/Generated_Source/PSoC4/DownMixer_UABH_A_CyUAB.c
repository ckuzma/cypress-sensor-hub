...
*   
* @internal
* @reglist @reg DownMixer_UABH_A_SRAM<tt>xx</tt>_REG<, where <tt>xx</tt> 
* spans 00 to 15.
*
* This function relies on several assumptions/definitions:
* @li The SRAM clocks are all stored contiguously in memory.
* @li The lowest address is DownMixer_UABH_A_UAB0_SRAM00.
...
* @endinternal
*
* @param uabSram starting address of UAB half waveform SRAM; ie DownMixer_UABH_A_SRAM1_PTR
* @param pattern clocking in LSbit to MSbit ordering for step 0 to step 15.
* When the @p modbitPattern is high, the
* modbit waveform (chosen by DownMixer_UABH_A_SetModbitSource()) is used as
